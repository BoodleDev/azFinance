...
type
  TdtmLCD = class(TDataModule)
    ...
    dsContractRevisions: TADODataSet;
    dtsContractRevisions: TDataSource;
    ADODataSet1: TADODataSet;
    cmdConvertContract: TADOCommand;
    cmdNewContract: TADOCommand;
    cmdUpdateInterest: TADOCommand;
    dsLoanContractOLD: TADODataSet;
    dsLoanContractRecordID: TIntegerField;
    dsLoanContractLoanID: TIntegerField;
    dsLoanContractRevisionNo: TIntegerField;
    dsLoanContractTransactionDate: TDateTimeField;
    dsLoanContractTransactionType: TStringField;
    dsLoanContractTransTypeCode: TStringField;
    dsLoanContractDescription: TStringField;
    dsLoanContractDebit: TBCDField;
    dsLoanContractCredit: TBCDField;
    dsLoanContractAmount: TBCDField;
    dsLoanContractBalance: TBCDField;
    dsLoanContractRate: TBCDField;
    dsLoanContractInterest: TBCDField;
...
//    procedure AddRecToContract(inType,PayNo:integer; EffectDate :TDate; Amount, ReCurBal :Currency; IntRate :real);
...
//    function GetNextInstalmentDate(inDate: TDate; CycleDay: Integer): TDate;
  public
    { Public declarations }
...
begin
  ...
  if not Ready then
  begin
    dsLoanContract.Active := Ready;
    dsContractRevisions.Active := Ready;
  end;
end;
...
begin
  ...
  if Assigned(dsLoanContract.Connection) then
  begin
    dsLoanContract.Close;
    dsLoanContract.Parameters[0].Value := LoanID;
    dsLoanContract.Open;

    dsContractRevisions.Close;
    dsContractRevisions.Parameters[0].Value := LoanID;
    dsContractRevisions.Open;
  end;
end;
...
//  else
//    Result := DateToEndOfMonth(IncMonth(inDate,1));
//  end; // case
//end;

//function TdtmLCD.GetNextInstalmentDate(inDate: TDate; CycleDay: Integer): TDate;
//begin
//  case qryLDS.FieldByName('LTaccT').AsInteger of
//    5,6 : Result := inDate + 7; // weekly instalment
//    7,8 : Result := inDate + 14;// bi-weekly instalment
//  else
//  begin
//    inDate := IncMonth(inDate,1);
//    Result := inDate + (Min(DaysInMonth(inDate),CycleDay) - DayOfTheMonth(InDate));
//  end;
//  end; // case
//end;
...
//    tmpDate := GetNextInstalmentDate(tmpDate, qryLDS.FieldByName('CycleEndDay').AsInteger);
//  end;
//end;
...
//function TdtmLCD.ValForDate(inDate: TDate; inType: integer; FldName: string): Currency;
//begin
//  Result := 0.00;
//  try
//    cdsContract.DisableControls;
...
begin
  ...
  dsLoanContract.Connection := conDB;
  dsContractRevisions.Connection := conDB;
  cmdNewContract.Connection := conDB;
end;

procedure TdtmLCD.CreateNewContract;
begin
  cmdNewContract.Parameters[0].Value := LoanID;
  cmdNewContract.Execute;

  cmdUpdateInterest.Parameters[0].Value := LoanID;
  cmdUpdateInterest.Execute;
end;
