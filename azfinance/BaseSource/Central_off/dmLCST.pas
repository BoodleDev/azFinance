...
type
  TdtmLCST = class(TDataModule)
    ...
    cdsContractLoanAdvance: TCurrencyField;
    cdsContractPayableInstallment: TCurrencyField;
    qryLoanAdvance: TADOQuery;
...
begin
  ...
  if IntRate = -5 then
    cdsContract.FieldByName('PayableInstallment').AsCurrency := ReCurBal;  
  if inType = 10 then
  begin
    cdsContract.FieldByName('LoanAdvance').AsCurrency := Amount;
    cdsContract.FieldByName('Balance').AsCurrency := Amount;
  end
  else
...
var
  i : integer;
  lTempDate : TDateTime;
  lInstallmentAmount, lPayableAmount : Currency;
begin
  tmpDate := qryLDS.FieldByName('FirstInsDate').AsDateTime;
  lTempDate := qryLDS.FieldByName('StartDate').AsDateTime;

  for i := 0 to qryLDS.FieldByName('LoanPeriod').AsInteger-1 do
  begin
    qryLoanAdvance.Active := False;
    qryLoanAdvance.Parameters[0].Value := qryLDS.FieldByName('LoanID').AsInteger;
    qryLoanAdvance.Parameters[1].Value := lTempDate;
    qryLoanAdvance.Parameters[2].Value := tmpDate;
    qryLoanAdvance.Active := True;
    if qryLoanAdvance.RecordCount > 0 then
    begin
      AddRecToContract(ttNorI,i+1,tmpDate,-1*qryLoanAdvance.FieldByName('AdvanceInstallment').AsCurrency,qryLoanAdvance.FieldByName('AdvancePayInstallment').AsCurrency,-5);
      lInstallmentAmount := qryLoanAdvance.FieldByName('AdvanceInstallment').AsCurrency;
      lPayableAmount := qryLoanAdvance.FieldByName('AdvancePayInstallment').AsCurrency;
    end
    else
      AddRecToContract(ttNorI,i+1,tmpDate,-1*lInstallmentAmount,lPayableAmount,-5);
...
begin
  ...
  begin
    ...
    begin
      ...
      ReCurBal := ReCurBal + cdsContract.FieldByName('AmtToAdd').AsCurrency + cdsContract.FieldByName('LoanAdvance').AsCurrency;
    end; {if}
    //if cdsContract.FieldByName('LoanAdvance').AsCurrency = 0 then
    //begin
  ...
  cdsContract.Edit;
  cdsContract.FieldByName('Amount').AsCurrency := cdsContract.FieldByName('Amount').AsCurrency + EndBal;
  cdsContract.FieldByName('AmtToAdd').AsCurrency := cdsContract.FieldByName('Amount').AsCurrency;
...
begin
  cdsStruc.Append;
  cdsStruc.FieldByName('LoanID').AsInteger      := LoanID;
  cdsStruc.FieldByName('EffectDate').AsDateTime := EffectDate;
  cdsStruc.FieldByName('Type').AsInteger        := inType;
  cdsStruc.FieldByName('TypeCnt').AsInteger     := TypeCnt;
  cdsStruc.FieldByName('Amount').AsCurrency     := Amount;
  cdsStruc.Post;
end;
...
begin
  ...
  qryLoanAdvance.Connection := conDB;
end;
...
procedure TdtmLCST.AddAdvances;
Var
  lSQL : String;
begin
  lSQL := 'SELECT * FROM lsLDLoanAdvance WHERE LoanID = '+IntToStr(LoanID);
  qryToUse.Active := False;
  qryToUse.SQL.Text := lSQL;
  qryToUse.Active := True;
  While Not qryToUse.Eof Do
  Begin
    AddRecToContract(10,-1,qryToUse.FieldByName('AdvanceDate').AsDateTime,
      qryToUse.FieldByName('AdvanceAmount').AsFloat,0,0);
    qryToUse.Next;
  End;
end;
