...
begin
  pcWizard.Align := alNone;
  pcWizard.Anchors := [akLeft,akRight,akTop,akBottom];
  pcWizard.Top := pcWizard.Top - 25;
  pcWizard.Height := pcWizard.Height + 25;
  pcWizard.Width := Self.Width;
  pcWizard.Style := tsButtons;
  pcWizard.MultiLine := False;
...
begin
  ...
  {  if Assigned(CurrentTab) then
   ...
   { CurrentTab := pcWizard.Pages[PageIndex];
    ...
    lblTitle.Caption := pcWizard.ActivePage.Caption;
  ...
  {  if CurrentTab.ImageIndex > -1 then
    ...
    btnNext.Enabled := not (PageIndex + 1 = pcWizard.PageCount);
  end;
end;
