...
type
  TfrmBase_Security = class(TForm)
    ...
    procedure grdControlDblClick(Sender: TObject);
    procedure DBGrid3DblClick(Sender: TObject);
    procedure FormDestroy(Sender: TObject);
    procedure grdControlTitleClick(Column: TColumn);
...
begin
  ...
  begin
    SecurityForm := TForm(Master);
    TForm(Parent).Caption := 'Security setup - ' + SecurityForm.Caption;
    //pnlSetup.Width := 150;
    TForm(Parent).Left := SecurityForm.Left + 20;
    TForm(Parent).Top  := SecurityForm.Top;
    TForm(Parent).Height := SecurityForm.Height;
    TForm(Parent).Width := SecurityForm.Width;// + pnlSetup.Width;
  end;
  ...
  begin
    ...
    else if Master.Controls[I] is TSpeedButton then
    begin
      Control := TSpeedButton.Create(Self);
      TSpeedButton(Control).Caption := TSpeedButton(Master.Controls[I]).Caption;
      TSpeedButton(Control).OnMouseDown := AddControl;
    end
...
begin
  if Master is TForm then
  begin
    if Assigned(SecurityForm) then
      if Master.Name <> SecurityForm.Name then
        Exit;
    SecurityForm := TForm(Master);
    Self.Caption := 'Security Setup - ' + SecurityForm.Caption;
  ...
  begin
    ...
    if Master.Controls[I].Tag = 0 then
...
begin
  ...
  dtmSecurity.cdsControls.FieldByName('Form').AsString := SecurityForm.Name;
...
begin
  ...
  begin
    Display := TSpeedButton(Control).Caption;
    Typ := Button;
  end
...
procedure TfrmBase_Security.grdControlDblClick(Sender: TObject);
begin
  btnAdd.Click;
end;

procedure TfrmBase_Security.DBGrid3DblClick(Sender: TObject);
begin
  btnRemove.Click;
end;

procedure TfrmBase_Security.FormDestroy(Sender: TObject);
begin
  SecurityForm := nil;
end;

procedure TfrmBase_Security.grdControlTitleClick(Column: TColumn);
begin
  dtmSecurity.cdsControls.IndexFieldNames := Column.FieldName;
end;

procedure TfrmBase_Security.Button1Click(Sender: TObject);
var
  Output: TextFile;
  S: String;
begin
  AssignFile(Output,'C:\SecurityControls.txt');
  Append(Output);
  dtmSecurity.cdsControls.First;
  Writeln(Output,SecurityForm.Name);
  while not dtmSecurity.cdsControls.Eof do
  begin
    S := ',' + dtmSecurity.cdsControls.FieldByName('Control').AsString + ',' +
      dtmSecurity.cdsControls.FieldByName('Caption').AsString + ',,';
    Writeln(Output,S);
    dtmSecurity.cdsControls.Next;
  end;
  Writeln(Output,' ');
  CloseFile(Output);
end;
