...
uses
  ...
  dbClient, ComCtrls, DBTables, ADODB, dbGrids, Variants, ShlObj,
  ActiveX, DBStyleGrid, Printers;
...
function MonthInWords(iMonth: Integer): String;
...
procedure InitializeModule(var ApplicationName, PersonalFolder: String);
procedure MoveToCurrent(TblName:TDataSet; sField:string; SendDate:TDateTime);
procedure OnlyOneApp;
procedure ReadFormDefaults(Form: TForm; IniFile: String);
procedure ReadFormGrids(Form: TForm; AppName, Folder: String);
...
procedure WriteFormDefaults(Form: TForm; IniFile: String);
...
procedure WriteFormGrids(Form: TForm; AppName, Folder: String);
function IsStrANumber(const S: string): Boolean;
function ValidCellNo(cellNo: string): boolean;
...
begin
  Result := ExtractFileDir(Application.ExeName) + '\';  // directory ends in '..\' i.e. 'C:\Develop\'
end;
...
function GetLoanContractBal(IntRate, PayAmt: real; iPeriods, PayTime: integer; StartDate: TDateTime): real;
var
...
begin
  ...
  begin
    ...
    try
      if Length(yr) < 4 then
        yr := Copy(FormatDateTime('yyyy', Date),1,4 - Length(yr)) + yr;
...
var
  IniFile: TIniFile;
begin
//ReadIni File for String given - Section , ID entry
  IniFile := TIniFile.Create(sIniName);
  Result  := IniFile.ReadInteger(sSection,sIDent,DefVal);
  IniFile.Free;
end;
...
procedure FilterDataSet(DataToFilter: TDataSet;sFilter:string);
begin
//Filter any given dataset with given filter
...
begin
  ...
  ApplicationName := LeftStr(ApplicationName, Pos('.',ApplicationName) -1);

  try
    PersonalFolder := GetSystemPath(CSIDL_PERSONAL) + '\AccZone';
    if not DirectoryExists(PersonalFolder) then
      CreateDir(PersonalFolder);
  except
    MessageDlg('An error occurred while trying to initialize your personal directory. ' +
      'Your settings will not be loaded/saved.', mtWarning, [mbOK], 0);
    PersonalFolder := '';
  end;
end;
...
begin
  ...
  try
    ...
    if (GetLastError() = ERROR_ALREADY_EXISTS) or
        ...
        (fMutex= 0) then
    begin
      MessageDlg('You can only have one instance of ' + Application.title +' running at one time. ' +
        #10#13  + 'This application will now terminate.',mtInformation,[mbOK],0);
...
var
  Defaults: TInifile;
begin
  Defaults := TInifile.Create(IniFile);
  try
    if Defaults.ReadInteger(Form.Name,'WindowState',0) = 1 then
      Form.WindowState := wsMaximized
    else
    begin
      Form.WindowState := wsNormal;
      Form.Left   := Defaults.ReadInteger(Form.Name, 'Left', (Screen.Width - Form.Width) div 2);
      Form.Top    := Defaults.ReadInteger(Form.Name, 'Top', (Screen.Height - Form.Height) div 2);
      Form.Width  := Defaults.ReadInteger(Form.Name, 'Width', Form.Width);
      Form.Height := Defaults.ReadInteger(Form.Name, 'Height', Form.Height);
    end;
  finally
    FreeAndNil(Defaults);
  end;
  ...
  begin
    try
      ...
      begin
        ...
        begin
          Filename := Folder + '\' + AppName + '_' + Form.Name + '_' +
            Form.Components[I].Name + '.grd';
          if FileExists(Filename) then
          begin
           // Cols := TDBGridColumns.Create();
//            ColCount := TDBGrid(Form.Components[I]).Columns.Count;
            TDBGrid(Form.Components[I]).Columns.LoadFromFile(Filename);
            {if TDBGrid(Form.Components[I]).Columns.Count <> Cols.Count then
            begin
              //TDBGrid(Form.Components[I]).Columns.RebuildColumns;
              //TDBGrid(Form.Components[I]).Columns.RestoreDefaults;
            end; }
          end;
        end;
      end;
    ...
    begin
      if Form.Components[I] is TDBGrid then
        for J := 0 to TDBGrid(Form.Components[I]).Columns.Count - 1 do
          TDBGrid(Form.Components[I]).Columns[J].Width :=
            Ini.ReadInteger(Form.Name + Form.Components[I].Name, IntToStr(J),
              TDBGrid(Form.Components[I]).Columns[J].Width);
    end;
  finally
    FreeAndNil(Ini);
...
procedure RunQuery(qryToRun: TQuery; inSQL: string; OorE: char);
begin
// OorE probably stands for "Open or Execute" - Christian
  qryToRun.Close;
  qryToRun.SQL.Clear;
...
begin
  ...
  try
    if Form.WindowState = wsMaximized then
      Defaults.WriteInteger(Form.Name,'WindowState',1)
    else
    begin
      Defaults.WriteInteger(Form.Name,'WindowState',0);
      Defaults.WriteInteger(Form.Name,'Left', Form.Left);
      Defaults.WriteInteger(Form.Name,'Top', Form.Top);
      Defaults.WriteInteger(Form.Name,'Width', Form.Width);
      Defaults.WriteInteger(Form.Name,'Height', Form.Height);
    end;
  finally
    FreeAndNil(Defaults);
  end;
 ...
 { if Prompt then
  ...
  begin }
    try
      ...
      MessageDlg('A new update for this module has been loaded', mtInformation, [mbOK], 0);
    finally
        ...
        begin
          ...
          if Assigned(DBGrid.DataSource) then
            if DBGrid.DataSource.DataSet.Active then
              DBGrid.Columns.SaveToFile(Folder + '\' + AppName + '_' +
                Form.Name + '_' + Form.Components[I].Name + '.grd');
        end;
      end;
    finally
...
begin
  try
    ...
    begin
      ...
      begin
        for J := 0 to TDBGrid(Form.Components[I]).Columns.Count - 1 do
          Ini.WriteInteger(Form.Name + Form.Components[I].Name,
            IntToStr(J), TDBGrid(Form.Components[I]).Columns[J].Width);
      end;
    end;
  finally
    FreeAndNil(Ini);
...
begin
  ...
  if not(IsStrANumber(cellNo))
    then passed := false
  else if ((LeftStr(cellNo,2) = '27') and (Length(cellNo) = 11))
    then passed := true
  else if ((LeftStr(cellNo,1) = '0') and (Length(cellNo) = 10))
    then passed := true
  else if ((Length(cellNo) = 10) and (LeftStr(cellNo,2) = '27'))
    then passed := false
  else if ((Length(cellNo) <> 10) and (Length(cellNo) <> 11))
    then passed := false;
