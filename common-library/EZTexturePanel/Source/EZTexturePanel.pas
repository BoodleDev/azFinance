...
type
  ...
  TEZTextureSettings = Class(TPersistent)
  private
...
begin
  if not FEZShape.Empty then begin
    if (GetWindowRgn(Hw,FRegion) <> 0) then DeleteObject(FRegion);
...
var
  ...
  procedure DoEvent;
  begin
    if (X >= Width-8) and (Y >= Height-8) then
      SC := $F008
    else if (X < 8) and (Y < 8) then
      SC := $F004
    else if (X < 8) and (Y >= Height-8) then
      SC := $F007
    else if (X >= Width-8) and (Y < 8) then
      Sc := $F005
    else
      SC := 0;
    if FSettings.FSize and (SC > 0) then
    begin
       IsSizeing := true;
       //Pos := Point(X,Y);
       ReleaseCapture;
       Perform(WM_SysCommand, SC, 0);
       DrawChange(Self);
    end;
...
const
  SC_DragMove = $F012;
begin
  if FSettings.FMove and not (FSettings.FSize and (
    ((X >= Width-8) and (Y >= Height-8)) or
    ((X < 8) and (Y < 8)) or
    ((X < 8) and (Y >= Height-8)) or
    ((X >= Width-8) and (Y < 8))
    )) then
  begin
    ReleaseCapture;
...
//------------------------------------------------------------------------------
procedure TEZTexturePanel.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  CanResize: Boolean;
begin
  if FSettings.FSize then
  begin
    if ((X >= Width-8) and (Y >= Height-8)) or
      ((X < 8) and (Y < 8)) or
      ((X < 8) and (Y >= Height-8)) or
      ((X >= Width-8) and (Y < 8)) then
        CanResize := True
    else
      CanResize := False;
  end
  ...
  if CanResize then
  begin
    if ((X < 8) and (Y < 8)) or ((X >= Width-8) and (Y >= Height-8)) then
      Cursor := crSizeNWSE
    else
      Cursor := crSizeNESW;
  end
...
function TEZTexturePanel.UpdateShadeObjects:boolean;
  ...
  begin
    ...
    if Shadable(ClassN) then begin
      ...
      if (Comp.ClassName = 'TEZTexturePanel') then Result := 'EZTex';
    end;
  end;
  //------
  ...
  begin
    for I := 0 to Length(TS)-1 do begin
       if (Owner.FindComponent(TS[I].Name).ClassName = 'TEZTexturePanel') then begin
          with Owner.FindComponent(TS[I].Name) as TEZTexturePanel do TT := Settings.TextureType;
  ...
  begin
    ...
    if (S = 'TEZTexturePanel') then if (CurrAct <> saText)  then result := CurrAct else Result := saNone;
...
begin
  if (FSettings.FTType = ttBlend) then begin
    for I := 1 to Owner.ComponentCount-1 do begin
       if (Owner.FindComponent(Owner.Components[I].Name).ClassName = 'TEZTexturePanel') then begin
          with Owner.FindComponent(Owner.Components[I].Name) as TEZTexturePanel do TT := Settings.TextureType;
          if (TT = ttBlend) then with Owner.FindComponent(Owner.Components[I].Name) as TEZTexturePanel do UpdateTexture;
       end;
    end;
  end;
end;
...
begin
  if (Count > 0) then begin
    ...
    if FSettings.FSize then begin
      Temp.Canvas.Brush.Style := bsSolid;
      Temp.Canvas.Brush.Color := clBlack;//clBtnFace;
      Re1 := Rect(Width-8,Height-8,Width,Height);
      Re2 := Rect(Width-8,0,Width,8);
      Re3 := Rect(0,0,8,8);
      Re4 := Rect(0, Height-8,8,Height);
      Temp.Canvas.FillRect(Re1);
      Temp.Canvas.FillRect(Re2);
      Temp.Canvas.FillRect(Re3);
      Temp.Canvas.FillRect(Re4);
      Temp.Canvas.Pen.Color := FSettings.FBCol;
      if Ctl3D then
      begin
        Frame3D(Temp.Canvas,Re1,clBtnHighlight,clBtnShadow,1);
        Frame3D(Temp.Canvas,Re2,clBtnHighlight,clBtnShadow,1);
        Frame3D(Temp.Canvas,Re3,clBtnHighlight,clBtnShadow,1);
        Frame3D(Temp.Canvas,Re4,clBtnHighlight,clBtnShadow,1);
      end
      else
      begin
        Temp.Canvas.Rectangle(Re1);
        Temp.Canvas.Rectangle(Re2);
        Temp.Canvas.Rectangle(Re3);
        Temp.Canvas.Rectangle(Re4);
      end;
    end;
...
begin
 for I := 0 to Length(FSettings.EZShadeOb)-1 do begin
    ...
    case FSettings.EZShadeOb[I].Act of
      ...
      saShape   : if (Owner.FindComponent(FSettings.EZShadeOb[I].Name).ClassName = 'TEZTexturePanel')
...
var
  ...
  procedure GetVar;
  begin
    with Obj as TEZTexturePanel do if FSettings.Shape.Empty or (FSettings.Shape = nil) then B := False else B := True;
    with Obj as TEZTexturePanel do begin
      L := Left;
      T := Top;
      W := Width;
      H := Height;
